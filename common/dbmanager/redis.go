...
func InitRedis() {
	var err error
	host, err := confmanager.AppCfg.GetValue("redis", "host")
	if err != nil {
		log.Logger.Fatalln("read app.ini of redis-host is err:", err)
	}
	port, err := confmanager.AppCfg.Int("redis", "port")
	if err != nil {
		log.Logger.Fatalln("read app.ini of redis-port is err:", err)
	}
	// username, err := confmanager.AppCfg.GetValue("redis", "username")
	// if err != nil {
	// 	log.Logger.Fatalln("read app.ini of redis-username is err:", err)
	// }
	// password, err := confmanager.AppCfg.GetValue("redis", "password")
	// if err != nil {
	// 	log.Logger.Fatalln("read app.ini of redis-password is err:", err)
	// }
	poolSize, err := confmanager.AppCfg.Int("redis", "poolsize")
	if err != nil {
